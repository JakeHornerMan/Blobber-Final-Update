...
public class Player_Move : MonoBehaviour
{
    ...
    public enum State {ground, jump, fall, absorb, slide, death}
    ...
    void Start()
    {
        ...
        //FloorIsLava = GameManager.FloorIsLava;
        matDefault = sr.material;
        Score.scoreAmount = 0;
        isDeath = false;
        FindObjectOfType<Countup>().Over(false);
    }
    ...
    public void playerMove()
    {
        ...
        else if (DisableMovement == true || isDeath == true) {
    ...
    //Animation State Setters
        if (raycastHit.collider != null)
        {
            ...
            {
                rb.velocity = Vector2.up * (jumpVelocity * 1.5f);
                FindObjectOfType<Health>().DamageFIL();
            }
            else if (FIL.FloorIsLava == false) {
                FindObjectOfType<Score_Add>().pointSet = 0;
                FindObjectOfType<Score_Add>().points = 0;
            }
        }
        ...
        else if (isDeath == true && raycastHit2.collider != null || isDeath == true && raycastHit2.collider == null) 
        { 
            baseGravity = 0f;
            rb.gravityScale = baseGravity;
            dontMove = true;
        ...
        {
            rayColor = Color.red;
            if (rb.velocity.y < .1f && Slide == false)
            {
                action = State.fall;
                killable = true;
            }
            ...
            else if (rb.velocity.y > .1f && Slide == true || rb.velocity.y < .1f && Slide == true)
            {
                action = State.slide;
            }
            else if (Slide == false)
    ...
    IEnumerator WaitAndJump(float _waitTime)
    {
        ...
        JumpMultiplier();
        FindObjectOfType<Score_Add>().AddPoints();
